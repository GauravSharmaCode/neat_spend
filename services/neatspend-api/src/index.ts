...
app.use((err: Error & { statusCode?: number; status?: string }, req: Request, res: Response) => {
  const errorWithOperational = err as Error & { statusCode?: number; status?: string; isOperational?: boolean };
  ...
  const errorResponse: ApiError = {
    ...
    message: errorWithOperational.isOperational ? err.message : 'Something went wrong!',
